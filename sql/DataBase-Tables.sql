...
IF NOT EXISTS(
    ...
    JOIN sys.schemas s
    ON t.schema_id = s.schema_id
    WHERE t.name = 'ELECTRICITY_FLOWS_FORECAST'
    AND s.name = 'RAW_DATA'
)
BEGIN
    CREATE TABLE KAFKA_ELECTRICITY.RAW_DATA.ELECTRICITY_FLOWS_FORECAST(
        zone NVARCHAR(50),
        temporalGranularity NVARCHAR(10),
        unit NVARCHAR(10),
        datetime DATETIME2,
...
IF NOT EXISTS(
    ...
    JOIN sys.schemas s
    ON t.schema_id = s.schema_id
    WHERE t.name = 'TOTAL_LOAD_FORECAST'
    AND s.name = 'RAW_DATA'
)
BEGIN
    CREATE TABLE [KAFKA_ELECTRICITY].[RAW_DATA].[TOTAL_LOAD_FORECAST](
	    zone NVARCHAR(50),
	    datetime DATETIME2,
	    createdAt DATETIME2,
	    updatedAt DATETIME2,
